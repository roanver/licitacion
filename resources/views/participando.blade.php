@include('layout.nav', ['title'=>'Participando'])
@include('layout.nav_licitaciones', ['estado'=> 'participando',  'menu' => $menu ?? ''])
          ...
          <tr class="border-b-2">
            <td class="pr-3">
              <form id="form_{{ $licitacion->numero_cotizacion }}" action="{{ route('actualizarEstado', $licitacion->numero_cotizacion) }}" method="POST">
                  {{ csrf_field() }}
                  <select class="w-30 border border-2 border-gray-400 rounded "  id="estado_aphix_{{ $licitacion->numero_cotizacion }}" name="estado_aphix" >
